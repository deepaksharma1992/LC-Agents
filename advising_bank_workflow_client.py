...
def shipping_bill_node(AgentState):
    ...
    letter_of_credit_data = extracted_docs_dict.get(DocumentCategories.LETTER_OF_CREDIT, None)
    vector_db = Chroma(collection_name="pdf_documents", embedding_function=embeddings,
                       persist_directory=r"rag/chroma_db")
...
def deciding_agent_node(AgentState):
    ...
    # deciding_agent_llm.bind_tools()
    response = deciding_agent_llm.invoke(user_input)
    print(f"Response of deciding agent: {response} ")

    # Bind to email tool
    email_subject = "Deciding Agent Workflow Result"
    email_body = f"{response.content}"
    pdf_path = "output_docs/deciding_agent_result.pdf"  # Ensure PDF is generated beforehand
    recipient = "recipient@example.com"

    send_email_tool(email_subject, email_body, "None", recipient)
